...
for i = 1:numel(names)
    ...
    currents.(names{i}) = current(cuts(i, 1): cuts(i, 2));
    temporal            = currents.(names{i});

    currents.(names{i}) = temporal(1:lim_temp);



    subplot(numel(names), 1, i)
    plot(rel_time, currents.(names{i}))
    xlim([0, 1.6])
    ylim([-2, 5])
...
currents.clean = cleanSignal;
currents.didt  = didt;

save('currents.mat', 'currents')
